...
function dan(rating):
    ...
    tier = (rating - 800) // 200               # 与区间表逐行吻合
    if tier > 9:             return 9           # 段 9：顶端开区间，无上限
    return tier                                 # 段 1..8
```

验证（对照上表逐行）：1049→0、1050→1、1199→1、1200→2、1399→2、1400→3、1500→3、1599→3、1600→4、2399→7、2400→8、2599→8、2600→9、5000→9。

段内细分（下/中/上）作为 tooltip / 详情页浮层附加信息，主界面只显示段位即可：按该段实际区间 `[lo, hi)` 三等分，下/中/上 分别落在前/中/后三分之一。段 0 与段 9 不显示亚分级。
