...
围绕单一围棋项目的天梯积分系统，供朋友圈 &lt; 100 人使用。系统提供：
...
**本系统的设计意图是一个通用的 1v1 竞技游戏 Elo 积分骨架。** 围棋（含段位映射、K=16 等参数）是当前 MVP 的**具体实例**，但系统的所有核心机制 —— Elo 计算、段位/等级映射、对局录入、历史曲线、多人对比、排行榜、软删除、管理员控制台 —— 都**不依赖围棋特定语义**。

把本系统适配到任何其它 1v1 竞技游戏（国际象棋、桌游单挑、俱乐部网球、地下乒乓球圈、卡牌对决……）只需修改以下"表层"，**不需要触碰任何业务逻辑**：

| 适配点 | 当前实例（围棋） | 通用化方式 |
|---|---|---|
| 项目名 / 二进制名 | `go_ultra` | 改 `go.mod` 模块名、`README`、`start.bat` 命名、Cloudflare Tunnel 域名 |
| UI 文案 / 术语 | "对局"、"段位"、"等级分"、"段" | 修改前端文案常量或 i18n 字典 |
| **K 因子** | 16（围棋稳态，体现棋艺稳定）| 国象常用 32；电竞天梯 24–40；节奏越快 K 越大 |
| **起始分** | 1500（= 3 段中）| 通常 1000 / 1200 / 1500 |
| **段位映射** | §4.3 围棋段位表（1–9 段）| 替换 `domain/rank.go` 中的常量表（如 Bronze/Silver/Gold/Platinum/Diamond/Master）|
| 段位显示标签 | "1 段 / 2 段 … 9 段" | 改为 "Bronze / Silver / Gold / …" 或 "新手 / 进阶 / 高手 / …" 等任何分层标签 |

**为支撑"改常量即可换游戏"的目标，实现时遵循以下约束：**

- 所有游戏特有的常量（K 因子、起始分、段位边界、段位标签）**集中在 `server/internal/domain/elo.go` 和 `server/internal/domain/rank.go`**，禁止散落到 service/handler 层
...
- 数据库表名、字段名、API 路径用通用术语（`matches` / `players` / `rating` / `dan`），**不使用游戏专有词汇**

**通用性范围之外**（不在"改名即可"的承诺之内）：

- 团队对战、多人混战、排名赛、积分制（保持 1v1 假设；改这些需要数据模型与算法改写）
- 多游戏并行（一个系统实例服务一个游戏；多游戏需要外层 `game_id` 字段）
- 段位之外的分层（如赛季、分区、地区）

实施每个阶段都要回答："这个改动放在 domain/* 中是否能让换游戏时只改这一处？" —— 如果不行，要么把它移到 domain，要么文档化为"游戏专有"。
...
| 决策点 | 选择 | 说明 |
|---|---|---|
| 积分算法 | 经典 Elo，K = 16 | 参照围棋职业等级分，K 比国象 Elo 缓和一半 |
| 起始分 | 1500 | 对应 3 段（中） |
...
- **未入段处理**：rating &lt; 1050 时段位映射函数返回 `null`，不在数据库存"段位"字段。
...
| 7段 | 2300 | [2200, 2400) |
| 8段 | 2500 | [2400, 2600) |
| 9段 | 2600+ | rating ≥ 2600（无上限）|

实现：`dan(rating) = clamp((rating − 1050) // 200 + 1, 1, 9)`；rating &lt; 1050 返回 null。

段内细分（弱/中/强）作为 tooltip / 详情页浮层附加信息，主界面只显示段位即可：弱 = 段位起点；中 = 起点 + 50；强 = 起点 + 100。
...
- **段位徽章配色**：未入段灰、1-3 段蓝、4-6 段紫、7-8 段金、9 段红
